...
class Quicksort extends AnimationOptions {
  ...
  configureTimingOptions() {
    ...
    this.layers.forEach(layer => {
      ...
      if (toLayer <= 4) {
        ...
        this.layerDelays[layer].elements[referenceEl] = referenceElPathTime +
                                            this.layerDelays[layer].waitTime;
        this.layerDelays[layer].base += referenceElPathTime +
                                          this.timeStagger;
      ...
      this.fromArrays[layer].forEach((el) => {
        ...
        this.layerDelays[layer].elements[el] = pathTime + this.layerDelays[layer].waitTime;
        this.layerDelays[layer].base += pathTime + this.timeStagger;
  ...
  testAnimation() {
    ...
    let $layerBrackets, $layerPlusEls, $prevLayerEls;
    ...
    this.layers.forEach(layer => {
      setTimeout((() => {
        ...
        if (toLayer <= 4) {
          ...
          }).bind(this), this.layerDelays[layer].elements[referenceEl]);
        }
        ...
        for (let i = 0; i < fromArray.length; i++) {
          ...
          }).bind(this), this.layerDelays[layer].elements[el]);
        }
