	...
	var AlgorithmVisualization = function () {
	  ...
	  _createClass(AlgorithmVisualization, [{
	    ...
	    value: function sizeVisualizationArea() {
	      ...
	      var visAreaHeight = $(window).innerHeight();
	      var svgContHeight = visAreaHeight - parseInt(algorithmTitle.height()) - parseInt(algorithmTitle.css("padding-top")) - parseInt(algorithmTitle.css("padding-bottom"));
	
	      $('#' + visAreaId).width(windowWidth - 200);
	      $('#' + visAreaId).height(visAreaHeight);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Quicksort = function (_AnimationOptions) {
	  ...
	  _createClass(Quicksort, [{
	    ...
	    key: 'renderMergingLayers',
	    value: function renderMergingLayers() {}
	    ...
	    value: function renderSection(options) {
	      ...
	      options.fromArray.forEach(function (el) {
	        prevElements[el] = $('#' + _this4.createArrayElId(options.fromLayer, el));
	      });
	      ...
	      sectionChars.forEach(function (char) {
	        if (char === '[' || char === ']') {
	          _this4.drawingArea.append(_this4.createArrayBracket({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset,
	            bracketType: char,
	            layer: options.toLayer
	          }));
	          layerLeftOffset += _this4.bracketWidth;
	        } else if (char === '+') {
	          _this4.drawingArea.append(_this4.createPlusSign({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset,
	            layer: options.toLayer
	          }));
	          layerLeftOffset += _this4.plusWidth;
	        } else if (char === ' ') {
	          _this4.drawingArea.append(_this4.createSpaceEl({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset
	          }));
	          layerLeftOffset += _this4.spaceWidth;
	        } else {
	          // must be an array element at this point
	          var newEl = _this4.createArrayElement({
	            layer: options.toLayer,
	            val: parseInt(char),
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset,
	          ...
	          _this4.drawingArea.append(newEl);
	          layerLeftOffset += _this4.elementWidth;
	        }
	      });
	    }
	...
	var AnimationOptions = function () {
	  ...
	  _createClass(AnimationOptions, [{
	    ...
	    value: function calibrateSizings() {
	      var visAreaHeight = $(window).innerHeight() - parseInt(this.algorithmTitle.height()) + parseInt(this.algorithmTitle.css("padding-top")) + parseInt(this.algorithmTitle.css("padding-bottom"));
