	...
	var AlgorithmVisualization = function () {
	  ...
	  _createClass(AlgorithmVisualization, [{
	    ...
	    value: function quickSort() {
	      ...
	      new _quicksort2.default(AlgorithmVisualization.OPTIONS);
	    }
	  }]);
	  ...
	  _createClass(Quicksort, [{
	    ...
	    value: function renderSection(options) {
	      ...
	      options.fromArray.forEach(function (el) {
	        prevElements[el] = $('#' + _this3.createArrayElId(options.fromLayer, el));
	      });
	      ...
	      sectionChars.forEach(function (char) {
	        if (char === '[' || char === ']') {
	          _this3.drawingArea.append(_this3.createArrayBracket({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset,
	            bracketType: char,
	            layer: options.toLayer
	          }));
	          layerLeftOffset += _this3.bracketWidth;
	        } else if (char === '+') {
	          _this3.drawingArea.append(_this3.createPlusSign({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset,
	            layer: options.toLayer
	          }));
	          layerLeftOffset += _this3.plusWidth;
	        } else if (char === ' ') {
	          _this3.drawingArea.append(_this3.createSpaceEl({
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset
	          }));
	          layerLeftOffset += _this3.spaceWidth;
	        } else {
	          // must be an array element at this point
	          _this3.drawingArea.append(_this3.createArrayElement({
	            layer: options.toLayer,
	            val: parseInt(char),
	            topOffset: topOffset,
	            leftOffset: layerLeftOffset
	          }));
	          layerLeftOffset += _this3.elementWidth;
	        }
	      });
	    }
	...
	var AnimationOptions = function () {
	  ...
	  _createClass(AnimationOptions, [{
	    ...
	    value: function calibrateSizings() {
	      ...
	      this.elementRadius = Math.floor(this.elementRadiusFactor * this.elementHeight);
	      this.fontSize = Math.floor(this.fontSizeFactor * this.elementHeight);
	
	      this.bracketSize = Math.floor(this.elementHeight * 0.9), this.bracketWidth = Math.floor(this.elementWidth / 3);
	    ...
	    value: function createAnimation(options) {
	      var myPath = _animejs2.default.path(options.pathIdName);
